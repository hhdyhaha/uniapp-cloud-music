	...
	<view class="content">
		<view class="content">
			...
			<view class="layout">
				...
				<icon type="clear" size="20" class="clear" @click="doClear(1)" v-show="account && account.length>0"></icon>
			</view>
			<view class="layout">
				<input password="true" v-model="password" placeholder="请输入密码" class="input_css" />	
				<icon type="clear" size="20" class="clear" @click="doClear(2)" v-show="password && password.length>0"></icon>
			</view>
			
			<view v-if="loginInfo && loginInfo.length>0">
				<view class="size_m_s tc999" @click="()=>{showLoginInfo = !showLoginInfo}">{{showLoginInfo ? '常用账号列表':'选择常用账号'}}</view>
				<view v-if="showLoginInfo">
					<view class="size_m_s margin_bottom_10 tc333" v-for="(item,index) in loginInfo" v-bind:key="index" @click="doSelectLoginInfo(item)">
...
<script>
	import {getLoginInfo} from '@/http/api/login.js';
	const app = getApp();
	export default{
		data(){
			return{
